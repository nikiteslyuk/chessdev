...
class PromoMenu:
    def __init__(self,col,to_sq):
        ...
        x=f*SQ; y=r*SQ+SQ if r*SQ+SQ+H<=SQ*8 else r*SQ-H
...
def push_clear(mv):
    ...
    board.push(mv); last=mv; pending=None
    legal_sqs.clear(); capture_sqs.clear()

def result_text():
    if board.is_checkmate():
        return "Мат. Белые победили" if board.turn==chess.BLACK else "Мат. Чёрные победили"
    return "Пат. Ничья"
...
while running:
    ...
    for e in pygame.event.get():
        ...
        if anims: continue      # пока идёт анимация
        ...
        if e.type==pygame.MOUSEBUTTONDOWN and e.button==1:
            ...
            if mv and board.is_castling(mv):
                ...
                anims.append(Anim(rook.piece_type,rook.color,(rs[0]-SQ//2,rs[1]-SQ//2),
                                  (rtg[0]-SQ//2,rtg[1]-SQ//2),rf))
            pending=mv
            drag_sq=drag_pos=None
            legal_sqs.clear(); capture_sqs.clear()
    ...
    if anims:
        ...
        if not board.turn and not promo and not board.is_game_over():
            ...
            if mv:
                ...
                anims.append(Anim(board.piece_at(mv.from_square).piece_type,board.turn,
                                  (s[0]-SQ//2,s[1]-SQ//2),(t[0]-SQ//2,t[1]-SQ//2),mv.from_square))
    ...
    for r in range(8):
        for f in range(8):
            pygame.draw.rect(screen, COL_L if (f+r)&1 else COL_D, pygame.Rect(f*SQ,r*SQ,SQ,SQ))
    if not game_over:
        ...
        for s in legal_sqs:
            f,r=chess.square_file(s),7-chess.square_rank(s); screen.blit(S_MOVE,(f*SQ,r*SQ))
        for s in capture_sqs:
            f,r=chess.square_file(s),7-chess.square_rank(s); screen.blit(S_CAP,(f*SQ,r*SQ))
        if board.is_check():
            k=board.king(board.turn); f,r=chess.square_file(k),7-chess.square_rank(k)
    ...
    if promo_pending:
        ...
        f, r = chess.square_file(pending.to_square), 7 - chess.square_rank(pending.to_square)
        screen.blit(SPR[(p.color, p.piece_type)], (f * SQ, r * SQ))
    ...
    if game_over: # полумаска + текст
        ...
        txt=result_text(); img=font.render(txt,True,(255,255,255))
