...
def play_game_pygame(
    ...
    class PromoMenu:
        def __init__(self, col, to_sq):
            ...
            x = f * SQ
            y = r_p * SQ + SQ if r_p * SQ + SQ + H <= SQ * 8 else r_p * SQ - H
            self.rects = []
            self.top = (x, y)
        ...
        if with_anim and board.fen() != new_board.fen():
            ...
            if move:
                ...
                if board.is_castling(move):
                    rf, rt = (
                        (7, 5) if chess.square_file(move.to_square) == 6 else (0, 3)
                    )
                    rf, rt = chess.square(
    ...
    while running:
            ...
            if promo:
                if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
                    ...
                    if ptype:
                        ...
                        uci = pending.uci()
                        resp = send_recv(
                            sock, {"action": "move", "table_id": table_id, "uci": uci}
                        )
                        if resp["status"] == "ok":
                ...
                if pending:
                    ...
                    if board.piece_at(
                        ...
                        if pending:
                            ...
                            resp = send_recv(
                                sock,
                                {"action": "move", "table_id": table_id, "uci": uci},
                            )
                            if resp["status"] == "ok":
                                reload_board(with_anim=True)
                            pending = None
        ...
        for r in range(8):
            for f in range(8):
                ...
                pygame.draw.rect(
                    ...
                    pygame.Rect(f * SQ, draw_r * SQ, SQ, SQ),
                )
        if not game_over:
        ...
        if game_over:
            ...
            txt = (
                ...
                else ("Мат. Чёрные победили" if board.is_checkmate() else "Пат. Ничья")
            )
...
class ChessCmd(cmd.Cmd):
            ...
            for t in resp["data"]:
                if t["id"] == self.current_table and t["white"] and t["black"]:
                    ...
                    play_game_pygame(
                        ...
                        flip_board=flip,
                        quit_callback=self.on_quit_table,
    ...
    def do_join(self, arg):
        ...
        if len(args) < 2:
            ...
            return
        resp = send_recv(
            self.sock, {"action": "join", "table_id": int(args[0]), "color": args[1]}
        )
        print(resp["msg"])
        if resp["status"] == "ok":
