...
def play_game_pygame(table_id, sock, my_color=None, flip_board=False, quit_callback=None, username=None):
    ...
    def draw_labels(table_info):
        ...
        if my_color == 'white':
            my_name = username if username else white
            opp_name = black if black else "Ожидание чёрных"
        elif my_color == 'black':
            my_name = username if username else black
            opp_name = white if white else "Ожидание белых"
        else:
            my_name = white if white else "White"
            opp_name = black if black else "Black"
        ...
        # Верхний лейбл — по центру верхнего отступа
        if top_label:
        ...
        # Нижний лейбл — по центру нижнего отступа
        if bottom_label:
            ...
            if promo:
                if e.type==pygame.MOUSEBUTTONDOWN and e.button==1:
                    ...
                    if ptype:
                        ...
                        # проигрываем анимацию промоции, только после — отправка на сервер
                        push_move = pending
                        def finish_promo():
                            nonlocal pending, promo, board, last
                            board.push(push_move)
                            last = push_move
                            promo=None
                            pending=None
                            send_move(push_move)
                        anims.append(Anim(board.piece_at(pending.from_square).piece_type, board.turn,
                                          (sq_center(pending.from_square)[0]-SQ//2, sq_center(pending.from_square)[1]-SQ//2),
                                          (sq_center(pending.to_square)[0]-SQ//2, sq_center(pending.to_square)[1]-SQ//2),
                                          pending.from_square))
                        pending = None
                        promo = None
                continue
